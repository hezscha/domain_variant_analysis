...
#read in combined alignment of the clustalo MSA and the pfam seed alignment and make one 20x20 matrix (csv with subst counts) for each position in the alignment (across all sequences that are part of the MSA), only considering seqs from the human proteome. 
#I need to go through the combined aln so the positions for the 20x20 mats match to the positions of counting the freqs of WT aas (and Amelie wanted to compare the human proteome data and the pfam data in that regard)
...
#aa tally once for pfam seed alignment
for msa_pos in range(1,len_aln+1):
...
#aa tally once for human proteome instances	
for msa_pos in range(1,len_aln+1):
